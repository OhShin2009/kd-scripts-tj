...
function pre_install(){
    ...
    if [ "$os_virtual_machine" = "xen" -o "$os_virtual_machine" = "kvm" ]; then
		...
		if [ "$os_virtual_machine" = "openvz" ]; then
			os="2"
			os_str="OpenVZ"
		else
...
function create_cert(){
    ...
    openssl pkcs12 -export -inkey client.pem -in client.cert.pem -name "client" -certfile ca.cert.pem -caname "${my_cert_cn}"  -out client.cert.p12 -password pass:1q2w3e4r
}
